// API: presigned PUT URL so the browser uploads an example's reference image
// straight to Spaces. Mirrors the reference-images upload flow.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const safe = String(filename || "image")
      ...
      .slice(-80);
    const key = `brain-examples/${randomUUID()}-${safe}`;
